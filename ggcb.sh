#!/bin/bash
bin_dir="/home/vflorelo/bioinformatics/bin"
...
elif [ "${run_mode}" == "blastn-sens" ]
then
	comp_command="blastn -task blastn -word_size 4 -query sequence_2 -subject sequence_1 -out blast_comparison_file -outfmt \"6 qseqid sseqid qstart sstart qend send evalue score length\" -evalue $eval_cutoff"
fi
genome_lengths=""
...
do
  ...
  do
    ...
    block_upper_start=$(echo "$datablock" | awk 'BEGIN{FS="\t"}{print $4*0.02}')		#begin of subject
    block_upper_end=$(echo "$datablock"   | awk 'BEGIN{FS="\t"}{print ((($6-$4)+1)*0.02)}')	#end of subject minus start of subject
    block_lower_end=$(echo "$datablock"   | awk 'BEGIN{FS="\t"}{print ((($6-$5)+1)*0.02)}')	#end of subject minus end of query
    block_lower_start=$(echo "$datablock" | awk 'BEGIN{FS="\t"}{print ((($5-$3)+1)*0.02)}')	#end of query minus start of query
...
map_width=$(echo "$genome_lengths" | grep -vi [a-z] | sort -n | uniq | awk '{print $1*0.02}' | tail -n1)
