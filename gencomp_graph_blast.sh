...
file_datablock=`grep -v ^# "$genome_order_file" | grep -v ^$`
...
if [ "$run_mode" = blastn ] || [ "$run_mode" = BLASTN ]
then
...
elif [ "$run_mode" = tblastx ] || [ "$run_mode" = TBLASTX ]
then
...
do
  ...
  if [ "$comp_num" -eq "1" ]
  then
    /home/vflorelo/Dropbox/git/local_scripts/general_genomics/build_gen_map.sh $file_name_1 multi 0
  fi
  /home/vflorelo/Dropbox/git/local_scripts/general_genomics/build_gen_map.sh $file_name_2 multi $genome_preload
