...
plus_datablock=$(echo "$gff_str" | awk -v sf="${sf}" 'BEGIN{FS="\t"}{if($7=="+"){print $4*sf FS $5*sf FS $9}}')
plus_count=$(echo "$plus_datablock" | wc -l)
minus_datablock=$(echo "$gff_str" | awk -v sf="${sf}" 'BEGIN{FS="\t"}{if($7=="-"){print $4*sf FS $5*sf FS $9}}')
