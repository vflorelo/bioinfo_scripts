...
num_orgs=`cat Global_Genome_Order | wc -l`
org_list=`cat Global_Genome_Order`
...
do
  ...
  do
    ...
    proportion=`cat shared_pairs | wc -l | awk -v norm_size="$norm_size" '{print ($1/norm_size)*100}'`
    score_sum=`grep -wFf shared_pairs $pair_list_file | awk 'BEGIN{FS="\t"}{score_sum+=$8}END{print score_sum}'`
    echo -e "$org_name\t$sub_org_name\t$proportion\t$score_sum" | awk -v cutoff="$cutoff" -v midpoint1="$midpoint1" -v midpoint2="$midpoint2" '{FS="\t"}{if($3<cutoff){print $1"\tUnrelated\t"$2"\t"$3"\t"$4}else if($3>cutoff && $3<midpoint1){print $1"\tDistantly related\t"$2"\t"$3"\t"$4}else if($3>midpoint1 && $3<midpoint2){print$1"\tRelated\t"$2"\t"$3"\t"$4}else if($3>midpoint2){print $1"\tClosely_related\t"$2"\t"$3"\t"$4}}'
    rm conflictive_pairs repeated_proteins shared_pairs
  done
done > genome_network.tsv
